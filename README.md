...
- `DouyinWindowKeywords`：匹配抖音窗口的关键词。
- `CodexWindowKeywords`：匹配 Codex 窗口的关键词。
...
窗口切换通过 Win32 API 完成：枚举顶层窗口、按关键词匹配进程名或窗口标题、激活目标窗口，并向抖音窗口中心发送鼠标点击来播放或暂停。

## 注意事项

- 首版主要面向抖音 Windows 客户端，不保证浏览器版抖音可用。
- 暂停/播放依赖窗口中心点击，如果抖音客户端界面布局变化，可能需要手动测试。
- 如果切回 Codex 时没有回到正确窗口，请检查 Codex 窗口关键词是否匹配到了其他窗口。
